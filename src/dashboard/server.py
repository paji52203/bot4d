...
from .routers import brain, monitor, visuals, performance, ws_router, settings
...
class DashboardServer:
    ...
    def _create_app(self) -> FastAPI:
        ...
        @app.post("/api/system/wakeup")
        async def system_wakeup():
            """Manually trigger bot re-analysis."""
            if not self.bot_running:
                return JSONResponse(status_code=400, content={"error": "Bot is not running"})
            
            # Check if we have a callback wired (usually from StartManager)
            if hasattr(self, 'settings_router_instance') and self.settings_router_instance.reload_callback:
                asyncio.create_task(self.settings_router_instance.reload_callback())
                return {"status": "waking_up"}
            
            return JSONResponse(status_code=501, content={"error": "Wakeup callback not wired"})

        @app.post("/api/position/override")
        async def override_position(request_data: dict):
            """Manually override TP and/or SL on the active position."""
            strategy = getattr(self.dashboard_state, 'trading_strategy', None)
            if not strategy or not strategy.current_position:
                return JSONResponse(status_code=404, content={"error": "No active position"})

            new_tp = request_data.get('take_profit')
            new_sl = request_data.get('stop_loss')

            if new_tp is not None:
                strategy.current_position.take_profit = float(new_tp)
            if new_sl is not None:
                strategy.current_position.stop_loss = float(new_sl)

            # Persist updated position to disk
            if strategy.persistence:
                await strategy.persistence.async_save_position(strategy.current_position)

            # Broadcast to all dashboard WebSocket clients
            await self.dashboard_state._broadcast({
                "type": "position_override",
                "take_profit": strategy.current_position.take_profit,
                "stop_loss": strategy.current_position.stop_loss
            })
        ...
        # Settings Router
        self.settings_router_instance = settings.SettingsRouter(self.config, self.logger)
        app.include_router(self.settings_router_instance.router)
